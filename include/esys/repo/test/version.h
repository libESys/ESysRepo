...
#define ESYSREPO_T_VERSION_NUMBER (ESYSREPO_T_MAJOR_VERSION * 1000) + (ESYSREPO_T_MINOR_VERSION * 100) + ESYSREPO_T_RELEASE_NUMBER
#define ESYSREPO_T_BETA_NUMBER      1
#define ESYSREPO_T_VERSION_FLOAT ESYSREPO_T_MAJOR_VERSION + (ESYSREPO_T_MINOR_VERSION/10.0) + (ESYSREPO_T_RELEASE_NUMBER/100.0) + (ESYSREPO_T_BETA_NUMBER/10000.0)

// check if the current version is at least major.minor.release
#define ESYSREPO_T_CHECK_VERSION(major,minor,release) \
    (ESYSREPO_T_MAJOR_VERSION > (major) || \
    (ESYSREPO_T_MAJOR_VERSION == (major) && ESYSREPO_T_MINOR_VERSION > (minor)) || \
    (ESYSREPO_T_MAJOR_VERSION == (major) && ESYSREPO_T_MINOR_VERSION == (minor) && ESYSREPO_T_RELEASE_NUMBER >= (release)))
