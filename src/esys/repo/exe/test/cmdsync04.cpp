...
ESYSTEST_AUTO_TEST_CASE(CmdSync04)
{
    ...
    Result rresult = m_fix_cmd_sync.get_git()->get_last_commit(last_commit);
    ESYSTEST_REQUIRE_EQUAL(rresult.ok(), true);

    rresult = m_fix_cmd_sync.get_git()->reset_to_parent();
    ESYSTEST_REQUIRE_EQUAL(rresult.ok(), true);
    ...
    // Get the last commit
    rresult = m_fix_cmd_sync.get_git()->get_last_commit(new_last_commit);
    ESYSTEST_REQUIRE_EQUAL(rresult.ok(), true);
    ...
    rresult = m_fix_cmd_sync.get_git()->open(repo_path.normalize().make_preferred().string());
    ESYSTEST_REQUIRE_EQUAL(rresult.ok(), true);
